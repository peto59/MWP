    ...
    internal static class NetworkManagerCommonCommunication
    {
...
#elif LINUX
                        throw new Exception("Received encrypted only command on unencrypted channel");
#endif
                    break;
...
#elif LINUX
                        throw new Exception($"wrong order to establish cypher, required step: {CommandsEnum.RsaExchange}");
#endif
...
#elif LINUX
                            throw new Exception("Received empty aes key");
#endif
...
#elif LINUX
                        throw new Exception($"wrong order to establish cypher, required step: {CommandsEnum.AesSend}");
#endif
...
#elif LINUX
                            throw new Exception("Received empty IV or length on long data");
#endif
...
#elif LINUX
        internal static void SongRequestInfo(ref NetworkStream networkStream, ref ConnectionState connectionState, byte[] data, ref RSACryptoServiceProvider encryptor)
#endif
        {
        ...
        internal static void ArtistImageSend(ref NetworkStream networkStream, ref RSACryptoServiceProvider encryptor, ref Aes aes, long length, byte[] data, ref ConnectionState connectionState)
        {
            ...
            try
            {
                ...
                List<Artist> artists = StateHandler.Artists.Search(artistAlias);
                if (artists.Count > 1)
                {
                    int artistIndex = StateHandler.Artists.IndexOf(artists[0]);
                    StateHandler.Artists[artistIndex] =
        ...
        internal static void AlbumImageSend(ref NetworkStream networkStream, ref RSACryptoServiceProvider encryptor, ref Aes aes, long length, byte[] data, ref ConnectionState connectionState)
        {
            ...
            try
            {
                ...
                List<Album> albums = StateHandler.Albums.Search(album);
                if (albums.Count > 1)
                {
                    int albumIndex = StateHandler.Albums.IndexOf(albums[0]);
                    StateHandler.Albums[albumIndex] = new Album(albums[0], albumImagePath);
                }

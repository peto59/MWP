    ...
    internal sealed class TagManager : IDisposable
    {
        ...
        public byte[] Image
        {
            get
            {
                ...
                if (dataData != null)
                {
                    ...
                    return ms.ToArray() ?? MusicBaseClassStatic.Placeholder;
                }
            ...
            set
            {
                ...
                using (MemoryStream stream = new MemoryStream())
                {
                    ...
                    pics[0] = new TagLib.Picture(value);
        ...
        public void Save()
        {
            ...
            if (saveFlags.HasFlag(SongSave.Album) && tfile != null && s != null)
            {
                ...
                {
                    a = new Album(tfile.Tag.Album, MWP_Backend.DatatypesAndExtensions.Album.GetImagePath(tfile.Tag.Album, FileManager.Sanitize(FileManager.GetAlias(tfile.Tag.Performers.FirstOrDefault() ?? tfile.Tag.Performers[0]))));
                    a.AddSong(ref s);
            ...
            if (saveFlags.HasFlag(SongSave.Artist) && tfile != null && a != null && s != null)
            {
                ...
                foreach (string performer in tfile.Tag.Performers)
                {
                    ...
                    {
                        art = new Artist(alias, MWP_Backend.DatatypesAndExtensions.Artist.GetImagePath(FileManager.Sanitize(performer)));
                        art.AddSong(ref s);
                        if (saveFlags.HasFlag(SongSave.Album))
                        {
                            a.AddArtist(ref art);
